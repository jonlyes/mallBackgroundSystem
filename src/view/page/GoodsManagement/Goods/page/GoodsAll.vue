<template>
  <!-- 全部 -->
  <div>
    <Goods-Card ref="goodsCardRef" :ajaxOptions="ajaxOptions" :btnOption="btnOption"></Goods-Card>
  </div>
  <!-- 新增功能 -->
  <div>
    <!-- 侧边抽屉 -->
        ...
        <el-scrollbar height="w-[100%]">
          <el-form v-model="fromData">
            ...
            <el-form-item label="封面" label-width="90px">
              <div
                class="rounded w-100px h-100px flex justify-center items-center border border-width-[1px] border-dashed cursor-pointer hover:bg-gray-100"
            ...
            <el-form-item label="商品分类" label-width="90px">
              <el-select v-model="fromData.category_id" placeholder="请选择商品分类">
                <el-option v-for="item of goodsCardRef?.tableData?.cates" :value-key="item.id" :label="item.name"
                  :value="item.id" />
              </el-select>
            </el-form-item>
            <el-form-item label="商品描述" label-width="90px">
              <el-input type="textarea" v-model="fromData.desc"
                placeholder="选填，商品卖点简鼠，例如：此款商品美观大方 性价比高 不容错过"></el-input>
            </el-form-item>
...
// 新增商品
const addShopFn = () => {
  store.dispatch('addShop', fromData).then(res => {
    goodsCardRef.value.refresh(1, ajaxOptions.value)
...
const btnOption = ref([
  ...
  { name: '批量删除', type: 'danger', fn: () => { console.log(123); } },
  { name: '上架', type: 'default', fn: () => { console.log(123); } },
  { name: '下架', type: 'default', fn: () => { console.log(123); } },
])
